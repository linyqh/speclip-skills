...
> **⚠️ 不要使用 `rendervideo` 工具或 `drama_helper.py timeline` 做最终渲染。**
> timeline.json 的格式把音频作为独立条目，rendervideo 等工具不支持此格式的音频叠加，会导致成片无解说音频。
> 必须按照下方的 FFmpeg 流程手动渲染：先合并配音（阶段 B），再拼接成片（步骤 9）。
