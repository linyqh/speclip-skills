...
- **向量检索先召回，人工再定稿**：先用 `videoindex` / `videosearch` 缩小候选戏眼范围，再结合 transcript 与视觉分析做最终编辑判断
...
基于步骤 2 的剧情骨架和步骤 3 的视觉判断，用 `videosearch` 做候选召回。
...
- 检索结果只是候选，不是最终入选片段
- 必须回到 transcript 和 `visual_analysis*.json` 交叉核验，再决定是否入选
...
- **检索只负责召回，不代替判断**：最终是否入选，仍以 transcript、真实台词力度、视觉表演强度为准
...
- 选中时间段后，必须回看 transcript / 视觉分析确认不是误召回
